...
#%% Imports from Python
from numpy import argmax,expand_dims,inf,squeeze,tile,zeros,seterr,linspace
...
def plan_allocations(myClass):
    ...
    Tgrid = T0 * (1 + g) ** linspace(0, par.T, Alen)  # Technology level grid with correct length
    ...
    y0 = Tgrid * Amat * (kmat**alpha) * (squeeze(n0[:,1,:])**(1.0-alpha))  # Include Tgrid in production function
    ...
    while (diff > crit) and (iter < maxiter): # Iterate on the Bellman Equation until convergence.
        ...
        for p in range(0,klen): # Loop over the k-states.
            for j in range(0,Alen): # Loop over the A-states.
                ...
                # Macro variables.
                y = Tgrid[j] * Agrid[j] * (kgrid[p]**alpha) * (squeeze(n0[p,:,j])**(1.0-alpha))  # Include Tgrid in production function
    ...
    sol.y = Tgrid * Amat * (kmat**alpha) * (n1**(1.0-alpha))  # Include Tgrid in production function
